...
public class Board extends Parent {
	...
	public ShipPart getShipPart(int x, int y) {
		StackPane s = (StackPane) ((HBox) board.getChildren().get(x)).getChildren().get(y);
		return (ShipPart) s.getChildren().get(0);
	}
	
	public StackPane getStackPart(int x, int y) {
		return(StackPane) ((HBox) board.getChildren().get(x)).getChildren().get(y);
