...
public class ModeController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		for (int i = 0; i < imageViewArray.length; i++) {
			Image image = new Image(getClass().getClassLoader().getResource("mode" + i + ".png").toString());
	...
	//	buttonBarHBox.setStyle("-fx-background-color:dodgerblue");
	   ...
	   private void typeScene() {
		   try {
				BorderPane root = (BorderPane) FXMLLoader.load(getClass().getResource("../Type.fxml"));
				...
				gameMode.show();
				 ...
				 s.close();
				   System.out.println("hhdcbducbdc");
